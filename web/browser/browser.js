...
(() => {
  ...
  function status() {
    ...
    document.getElementById('device-name').textContent='Local processing · no account connection';
  ...
  function coverage(s) {
    ...
    `<section class="source-panel browser-intro"><h2>Your files stay on this computer</h2><p>No application backend, account connection, telemetry or runtime downloads. Only files you select are read. Conversation text is parsed in memory; the ledger keeps accounting metadata, source hashes and your declarations.</p>
    ${hosted?'<p class="note">You are using the web edition. The site host receives the request to open this page; your selected evidence is processed only in this browser. <a href="session-observatory-browser.html" download>Download the offline app</a> to use it without visiting a website. Browser storage belongs to this site; use a complete backup to move your workspace to the downloaded app.</p>':''}
