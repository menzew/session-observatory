...
GitHub's automatic source downloads do not include the built HTML. Make that distinction explicit so users do not need to install Node or Python just to try the app. Link [Browser guide](browser.md) and [Privacy](../PRIVACY.md) from release notes. Keep the collector download/setup instructions secondary.
...
Extract the ZIP into a fresh directory. From there, run the Python suite and `python3 -m collector --demo`; then install the declared browser development dependencies and run the browser suite. Check that demo mode uses fictional data and the app can start independently. Documentation screenshots must be captured in demo mode; never copy a screenshot from your live workspace.
...
The development repository is private. The public `menzew/session-observatory-web` repository contains only the generated site and offline download. GitHub Pages serves its `main` branch root; no application backend or credentials are deployed.

After the checks pass, run `npm run build:pages`. Publish only `index.html`, `session-observatory-browser.html`, its `.sha256`, `browser-runtime-manifest.json`, and `.nojekyll` from `dist/site/`. The generated HTML includes executable application code and license notices; those bytes are intentionally public. Never copy the working tree, evidence, or a saved workspace into the hosting repository.

The private repository's default Actions token cannot push another repository. Site publication is a separate maintainer push of verified build artifacts; no cross-repository credential is stored in this project. Verify the deployed page, fictional demo, offline-download link, and browser import/storage behavior after a publication.
