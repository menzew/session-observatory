...
Implementation: `engine/analytics.py`, `Ledger.filters()` in `engine/ledger.py`, and `web/analytics.js`. `/api/analytics` returns the shared aggregate model used by the view; API access retains the app's loopback and same-origin checks. Existing source configuration and immutable metering evidence are unchanged.
