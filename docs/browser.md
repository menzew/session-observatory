...
[Open the public web edition](https://menzew.github.io/session-observatory-web/) to try fictional data or import your own logs without downloading first. The host serves the page; selected evidence stays in your browser. Saving uses the hosted site’s browser storage. Use **Download the offline app** for a local HTML copy, and a complete backup to move an existing workspace between the two. See the [hosted privacy boundary](../PRIVACY.md#hosted-web-edition).
...
Default-home examples are `~/.codex/sessions` on Linux/macOS and `%USERPROFILE%\.codex\sessions` on Windows. Use the actual home configured for your installation if it differs. Hidden folders may need to be shown in the picker. See [Windows, WSL, and custom homes](windows.md).
...
Application source is Apache-2.0. Bundled components retain their licenses, including Pyodide MPL-2.0. Full texts and source references are included under **Sources & privacy → About, licenses and runtime sources** and in [third-party notices](../third_party/NOTICE.txt). External source links use the network only when you choose to open them.
