...
class BrowserBridgeTests(unittest.TestCase):
    def setUp(self):
        ...
        with patch('getpass.getuser'), patch('socket.gethostname'):
            spec = importlib.util.spec_from_file_location('browser_bridge', Path(__file__).resolve().parents[1] / 'web' / 'browser' / 'bridge.py')
    ...
    def test_native_evidence_migrates_without_changing_accounting_or_audit(self):
        ...
        try:
            ...
            with patch('engine.ledger.now', return_value='2099-01-01T00:00:00Z'):
