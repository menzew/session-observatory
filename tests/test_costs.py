...
from engine.costs import COMPONENTS, estimate, scenario_config
...
class CostTests(unittest.TestCase):
    ...
    def add(self,ident,project='/cost/A',model='gpt-5.6-sol',n=1000,native=True):
        # Fixed historical evidence avoids the exclusive report end sharing the
        # newest record's millisecond on fast runners.
        record=lambda kind,payload:line(kind,payload,'2026-01-01T00:00:00Z')
        content=record('session_meta',dict(id='task-'+ident,cwd=project,model_provider='openai'))
        content+=record('turn_context',dict(model=model))
        ...
        if native:content+=record('token_usage_record',dict(response_id=ident,usage=counts))
        else:content+=record('event_msg',dict(type='token_count',info=dict(total_token_usage=counts)))
