...
for(const name of ['ledger.py','analytics.py','costs.py','limits.py','demo.py'])assets[name]=(await read(name)).toString('base64');
assets['bridge.py']=(await read('browser/bridge.py')).toString('base64');
const notices=(await Promise.all(['browser/THIRD-PARTY.txt','LICENSE','NOTICE','browser/licenses/PYODIDE-MPL-2.0.txt','browser/licenses/PYTHON.txt','browser/licenses/EMSCRIPTEN.txt'].map(async p=>`\n\n===== ${p} =====\n${await read(p)}`))).join('');
const payload={assets,worker:(await read('browser/worker.js')).toString('base64'),notices:Buffer.from(notices).toString('base64')};
let html=(await read('static/index.html')).toString();
html=html.replace(/<script defer src="[^"]+"><\/script>/g,'');
html=html.replace('<link rel="stylesheet" href="/style.css">',`<style>${await read('static/style.css')}\n.browser-actions{display:flex;gap:10px;flex-wrap:wrap;margin:20px 0}.browser-intro h2{margin-top:0}.source-panel>p{max-width:90ch;line-height:1.6}.source-panel>summary{cursor:pointer}</style>`);
html=html.replace('href="/favicon.svg"',`href="data:image/svg+xml;base64,${(await read('static/favicon.svg')).toString('base64')}"`);
...
const scripts=['browser/browser.js','static/limits.js','static/analytics.js','static/costs.js','static/app.js'];
